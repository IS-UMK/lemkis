...
auto test_reduction(){
    ...
    std::array<fraction<int>, 9> m_array{{{7}, {2}, {4}, {5}, {5}, {3}, {7}, {4}, {9}}};
    std::array<fraction<int>, 9> reduced_array{{{7}, {2}, {4}, {0}, {25, 7}, {1, 7}, {0}, {0}, {123, 25}}};
...
auto test_steps(){
    matrix<fraction<int>> m{3, 3, {0}};
    std::array<fraction<int>, 9> m_array{{{7}, {2}, {4}, {5}, {5}, {3}, {7}, {4}, {9}}};
    std::vector<std::string> steps_test{"R2 - 5/7 * R1", "R3 - 1/1 * R1", "R3 - 14/25 * R2"};
    std::size_t iterator = 0;
    for(std::size_t i = 0; i < 3; i++){
        for(std::size_t j = 0; j < 3; j++){
            m[i, j] = m_array[iterator];
...
auto test_determinant(){
    ...
    auto reduced = utils::matrix::run(m);
    return testing::expect_equal(utils::matrix::determinant(m).value(), determinant_test);
}
...
auto test_inverse(){
    ...
    std::array<fraction<int>, 9> m_array{{{7}, {2}, {4}, {5}, {5}, {3}, {7}, {4}, {9}}};
    std::array<fraction<int>, 9> inverse_array{{{11, 41}, {-2, 123}, {-14, 123}, {-8, 41}, {35, 123}, {-1, 123}, {-5, 41}, {-14, 123}, {25, 123}}};
    std::size_t iterator = 0;
    for(std::size_t i = 0; i < 3; i++){
        for(std::size_t j = 0; j < 3; j++){
            m[i, j] = m_array[iterator];
