...
namespace benchmark {
    ...
    auto run_producers_stack_mutex(StackType& stack,
                                   std::atomic<int>& produced_count,
                                   int n_producers,
                                   int total_items,
                                   std::vector<std::jthread>& producers)
        ...
        for (int p = 0; p < n_producers; ++p) {
            producers.emplace_back([&]() {
                while (produced_count < total_items) {
                    stack.mutex_push(produced_count);
                    produced_count++;
                }
            });
        }
        ...
        template <typename StackType>
        auto run_producers_stack_cv(StackType & stack,
                                    int n_producers,
                                    int item_count,
                                    std::vector<std::jthread>& producers)
            -> void {
        ...
        auto run_producers_queue_mutex(QueueType & queue,
                                       int n_producers,
                                    int item_count,
                                       std::vector<std::jthread>& producers)
            -> void {}
        ...
        template <typename QueueType>
        auto run_producers_queue_cv(QueueType & queue,
                                    int n_producers,
                                    int item_count,
                                    std::vector<std::jthread>& producers)
            -> void {}
        ...
        template <typename StackType>
        auto run_consumers_stack_mutex(StackType & stack,
                                       int n_consumers,
                                    int item_count,
                                       std::vector<std::jthread>& producers)
            -> void {}
        ...
        template <typename StackType>
        auto run_consumers_stack_cv(StackType & stack,
                                    int n_consumers,
                                    int item_count,
                                    std::vector<std::jthread>& producers)
            -> void {}
        ...
        template <typename QueueType>
        auto run_consumers_queue_mutex(QueueType & queue,
                                       int n_consumers,
                                    int item_count,
                                       std::vector<std::jthread>& producers)
            -> void {}
        ...
        template <typename QueueType>
        auto run_consumers_queue_cv(QueueType & queue,
                                    int n_consumers,
                                    int item_count,
                                    std::vector<std::jthread>& producers)
            -> void {}
        ...
        template <typename StackType>
        auto benchmark_stack_mutex_new(const std::string& name,
                                       int n_producers,
                                    int item_count,
        ...
        template <typename StackType>
        auto benchmark_stack_cv_new(const std::string& name,
                                    int n_producers,
                                    int item_count,
                                    int total_items) -> void {
            StackType stack;
        ...
        // benchmark function for queue
        template <typename QueueType>
        static auto benchmark_lock_free_queue_new(
            ...
            std::print(
                "{} with {} mechanism: {} producers, {} consumers, {} items "
                "total "
                "- "
                "completed in {} ms\n",
                queue_name,
        ...
        auto run_new_benchmarks(int N) -> void {
            ...
            for (int const producers : {1, 2, 4}) {
                for (int const consumers : {1, 2, 4}) {
                    ...
                    benchmark_queue_new<two_stack_queue<int>>(
                        "two_stack_queue",
                        "mutex",
                        producers,
                        consumers,
            ...
            for (int const producers : {1, 2, 4}) {
                for (int const consumers : {1, 2, 4}) {
                    ...
                    benchmark_lock_free_queue_new<
                        moodycamel::ConcurrentQueue<int>>("LockFreeQueue",
                                                          "compare-exchange",
                                                          producers,
                                                          consumers,
                                                          items_per_producer);
