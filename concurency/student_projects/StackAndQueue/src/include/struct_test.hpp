...
namespace struct_test {
    ...
    auto demonstrate_dr(int item_count) -> void;
    template <typename SName>
    ...
    template <typename SName>
    auto demonstrate_conc(const std::string& name, int item_count) -> void;
    template <typename SName>
    ...
    auto conc_create_consumer(SName& safe_struct, std::atomic<int>& count)
        -> std::jthread;
    template <typename SName>
    auto conc_consumer_loop(SName& safe_struct,
                            std::atomic<int>& count,
    ...
    auto cv_create_producer(SName& safe_struct,
                            ...
                            std::atomic<int>& count) -> std::jthread;
    template <typename SName>
    auto cv_producer_loop(SName& safe_struct,
                          int item_count,
                          std::atomic<int>& count,
                          const std::stop_token& stoken) -> void;
    template <typename SName>
    auto cv_producer_work(SName& safe_struct, int i, std::atomic<int>& count)
        -> void;
    template <typename SName>
    ...
    auto wait(std::jthread& producer, std::jthread& consumer) -> void;
}  // namespace struct_test