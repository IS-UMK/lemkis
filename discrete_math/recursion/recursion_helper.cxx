...
*/
export inline auto is_safe(const std::vector<int>& board, int row, int col) -> bool {
    for (int i = 0; i < col; i++) {
...
*/
export inline void solve_n_queens_util(int n,
                                int col,
...
*/
export inline auto print_solutions_helper(int n,
                                   int i,
                                   auto solution) {
    for (int j = 0; j < n; j++) {
...
*/
export inline auto print_solutions(const std::vector<std::vector<int>>& solutions,
                            int n) {
...
*/
export inline auto get_max_chocolates(std::size_t wrappers, std::size_t wrap)
    -> std::size_t {
...
*/
export inline auto is_prime_helper(std::size_t number, std::size_t divisor) -> bool {
...
*/
export inline auto find_factors(std::size_t number,
                         std::size_t start,
...
*/
export inline auto find_combinations(std::size_t number,
...
*/
export inline auto generate_bit_representations(const std::string& current,
                                         int onesCount,
                                         int zerosCount,
                                         int remainingBits,
...
export inline auto bubble_pass(std::vector<int>& v, std::size_t n) -> void {
    ...
    for (std::size_t i = 0; i < n - 1; ++i) {
        if (v[i] > v[i + 1]) { std::swap(v[i], v[i + 1]); count++;}
    }
    if(count == 0){return;};
    bubble_pass(v, n - 1);
}
...
// NOLINTBEGIN
export inline auto find_sequences(const std::vector<int>& v0, const std::vector<int>& v1, int indexi, int indexj, std::vector<int>& current_sequence, std::vector<std::vector<int>>& sequences, bool fromV0) -> std::vector<std::vector<int>> {
    if (fromV0) {
        for (int i = indexi; i < v0.size(); ++i) {
            if (current_sequence.empty() || v0[i] > current_sequence.back()) {
                current_sequence.push_back(v0[i]);
                find_sequences(v0, v1, i + 1, indexj, current_sequence, sequences, false);
                current_sequence.pop_back();
            }
        }
    } else {
        for (int j = indexj; j < v1.size(); ++j) {
            if (v1[j] > current_sequence.back()) {
                ...
                find_sequences(v0, v1, indexi, j + 1, current_sequence, sequences, true);
                current_sequence.pop_back();
            }
        }
    }
