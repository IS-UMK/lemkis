...
class polynomial : public std::vector<T> {
  ...
  public:
    ...
    auto operator+=(const polynomial<T>& p) -> polynomial<T>& {
        if (p.degree() > degree()) { this->resize(p.degree() + 1, T{0}); }
        for (std::size_t i = 0; i <= p.degree(); ++i) { (*this)[i] += p[i]; }
        return trim_polynomial(*this);
    }
    ...
    auto operator*=(const T& val) -> polynomial<T>& {
        for (std::size_t i = 0; i <= degree(); i++) { (*this)[i] *= val; }
        return trim_polynomial(*this);
    }
...
template <typename T>
struct std::formatter<polynomial<T>, char> {

    template <class ParseContext>
    constexpr auto parse(ParseContext& ctx) -> ParseContext::iterator {
        auto pos = ctx.begin();
        if (pos == ctx.end()) { return pos; }
        if (*pos != '}') { ++pos; }
        return pos;
    }

    template <class FmtContext>
    auto format(polynomial<T> p,
